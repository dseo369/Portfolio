...
// specifies number of milliseconds per animation cycle
...
unsigned int    MoonTex;	    // the texture object
unsigned int    SpaceTex;	    // the texture object
unsigned int    DeathTex;	    // the texture object
GLdouble radius = 1;
int 	Distort = 0;
GLfloat	xRotated;
GLfloat	yRotated;
...
Display( )
{
	...
	// set the eye/camera position, look-at position, and up-vector:

	if( WhichView == SPACE ) {
		gluLookAt( 30., -10., 50., 	0., 0., 0.,		0., 1., 0. );
	}

	if( WhichView == DEATHSTAR ) {
		gluLookAt( -38., 18., -28., 	0., 0., 0.,		0., 1., 0. );
	}

	if( WhichView == SUN ) {
		glRotatef( (360.*Time)/2, 0., 1., 0. );
		gluLookAt(  5., 5., 5., 	-40., 20., -30.,		0., 1., 0. );
		glRotatef( (360.*Time)/2, 0., 1., 0. );
	}
	...
	// point light to simulate the sun's light pointing in all directions
	...
	glPushMatrix(); 
		if(WhichView != SUN) {
			...
			glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE); // use GL_REPLACE instead of GL_MODULATE so the sun is illuminated
			// glRotatef( (360.*Time)*10, 0., 1., 0. );
			// glTranslatef( 0., 0., 0 );
			glRotatef( (360.*Time)/2, 0., 1., 0. );
			OsuSphere( 8., 60, 60 );
...
InitMenus( )
{
	...
	glutAddMenuEntry( "On Earth",  	 EARTH );
	glutAddMenuEntry( "On the Sun",  SUN );
	glutAddMenuEntry( "On the Death Star", 	 DEATHSTAR );
	glutAddMenuEntry( "In Space", 	 SPACE );
...
InitGraphics( )
{
    ...
    Texture2 = BmpToTexture( "suntex.bmp", &width, &height );
    if( Texture2 == NULL )
            fprintf( stderr, "Cannot open texture '%s'\n", "suntex.bmp" );
    else
            fprintf( stderr, "Width = %d ; Height = %d\n", width, height );
	...
	Texture3 = BmpToTexture( "moontex.bmp", &width, &height );
    if( Texture3 == NULL )
            fprintf( stderr, "Cannot open texture '%s'\n", "moontex.bmp" );
    else
            fprintf( stderr, "Width = %d ; Height = %d\n", width, height );
	...
	Texture4 = BmpToTexture( "spacetex.bmp", &width, &height );
    if( Texture4 == NULL )
            fprintf( stderr, "Cannot open texture '%s'\n", "spacetex.bmp" );
    else
            fprintf( stderr, "Width = %d ; Height = %d\n", width, height );
	...
	Texture5 = BmpToTexture( "deathtex.bmp", &width, &height );
    if( Texture5 == NULL )
            fprintf( stderr, "Cannot open texture '%s'\n", "deathtex.bmp" );
    else
            fprintf( stderr, "Width = %d ; Height = %d\n", width, height );
